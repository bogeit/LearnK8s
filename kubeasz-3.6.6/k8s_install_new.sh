...
# 变量定义
export release=3.6.6  # 支持k8s多版本使用,定义下面k8s_ver变量版本范围: v1.31.2 v1.30.1 v1.29.5 v1.28.10 v1.27.14 v1.26.15
export k8s_ver=v1.32.3  # | docker-tag tags easzlab/kubeasz-k8s-bin   注意: k8s 版本 >= 1.24 时，仅支持 containerd
rootpasswd=$1
netnum=$2
nethosts=$3
cri=$4
...
# deploy机器检测python环境（针对centos7系统）
if cat /etc/redhat-release &>/dev/null;then
    python2 -V &>/dev/null
    if [ $? -ne 0 ];then
